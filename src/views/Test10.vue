<template>
  <div class="container-fluid">
    <div class="search-wrapper">
      ...
      <!-- check boxes -->
      <div id="checkboxes">
        <div v-for="(group,index) in groups" :key="index" class="form-check form-check-inline">
          <input
          ...
          >
          <label class="form-check-label">{{ group.value }}</label>
        </div>
      </div>
    </div>
    <!-- end of checkboxes -->
...
export default {
  ...
  computed: {
    selectedFilters: function() {
      let filters = [];
      let checkedFiters = this.groups.filter(obj => obj.checked);
    ...
    getfilteredData: function() {
      ...
      if (this.selectedFilters.length > 0) {
        filteredDataByfilters = this.filteredData.filter(obj =>
          this.selectedFilters.every(val => obj.group.indexOf(val) >= 0)
        );
      ...
      if (this.search !== "") {
        filteredDataBySearch = this.filteredData.filter(
          obj => obj.group.indexOf(this.search.toLowerCase()) >= 0
        );
